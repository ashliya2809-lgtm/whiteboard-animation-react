...
import { createProject, createScene, createDrawingModel, createTextModel, cloneScene } from '../models';
...
export const useStore = create((set, get) => ({
  ...
  resizeGraphic(graphicId, width, height) {
    set(state => produce(state, draft => {
      for (const scene of draft.project.scenes) {
        const g = scene.graphics.find(g => g.id === graphicId);
        if (g) { g.width = Math.max(20, width); g.height = Math.max(20, height); return; }
      }
    }));
  },
